...
std::unordered_map<int, Qt::KeyboardModifier> makeModifierMapping() {
    return {
        {MOD_WIN, Qt::MetaModifier},
        {MOD_ALT, Qt::AltModifier},
        {MOD_CONTROL, Qt::ControlModifier},
        {MOD_SHIFT, Qt::ShiftModifier}
    };
}

std::unordered_map<int, Qt::Key> makeKeyMapping() {
    return {
...
class HotkeyDispatcherPrivate : public QWindow {
public:
    ~HotkeyDispatcherPrivate() {
        qDeleteAll(notifierByHotkey_);
    }

    virtual bool nativeEvent(const QByteArray& eventType, void* message, long* result) override {
        MSG* msg = static_cast<MSG*>(message);
        if (msg->message == WM_HOTKEY) {
            auto pos = notifierById_.find(msg->wParam);
            if (pos != notifierById_.end())
                pos->second->hotkeyPressed();
        }

        return QWindow::nativeEvent(eventType, message, result);
    }

    HotkeyNotifier* HotkeyDispatcher::registerHotkey(const Hotkey& hotkey) {
        /*Qt::LeftButton

        
        BOOL ok = RegisterHotKey(reinterpret_cast<HWND>(winId()), nextId_, MOD_ALT, VK_PAUSE);
        QString s = GetLastErrorAsString();*/

        return nullptr;
    }

    void HotkeyDispatcher::unregisterHotkey(const Hotkey& hotkey) {
    }

public:
    int nextId_ = 1;
    HotkeyDispatcher* q = nullptr;
    QHash<Hotkey, int> idByHotkey_;
    QHash<int, HotkeyNotifier*> notifierById_;
};

static HotkeyDispatcher* HotkeyDispatcher_instance = nullptr;

HotkeyDispatcher* HotkeyDispatcher::instance() {
    return HotkeyDispatcher_instance;
}

HotkeyDispatcher::HotkeyDispatcher() :
    d(new HotkeyDispatcherPrivate())
{
    d->q = this;

    assert(HotkeyDispatcher_instance == nullptr);
    HotkeyDispatcher_instance = this;
}
...
HotkeyNotifier* HotkeyDispatcher::registerHotkey(const Hotkey& hotkey) {
    return d->registerHotkey(hotkey);
}

void HotkeyDispatcher::unregisterHotkey(const Hotkey& hotkey) {
    d->unregisterHotkey(hotkey);
}
